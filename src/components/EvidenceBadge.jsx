...
export default function EvidenceBadge({ post, compact = false }) {
  ...
  return (
    <span className="evidence-badge" data-level={evidence.level} data-freshness={evidence.freshness} title={environment || evidence.levelLabel}>
      <strong>{evidence.levelLabel}</strong>
      {evidence.freshness !== 'unknown' && <span> · {evidence.freshnessLabel}</span>}
