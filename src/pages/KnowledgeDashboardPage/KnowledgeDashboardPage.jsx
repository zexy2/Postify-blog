...
export default function KnowledgeDashboardPage(){
 ...
 return <div className={`container ${styles.page}`}>
  ...
  <section className={styles.section}><div className={styles.sectionTitle}><h2>{en?'Re-verification queue':'Yeniden doğrulama kuyruğu'}</h2><span>{data.posts.length}</span></div><div className={styles.queue}>{data.posts.length===0?<p>{en?'No authored knowledge yet.':'Henüz yazdığın bilgi yok.'}</p>:data.posts.map((post)=>{const evidence=getKnowledgeEvidence(asPost(post));const summary=summarizeCommunityEvidence(post.summary||{});const canReverify=hasMeaningfulEvidenceEntry(post.environment,3)&&hasMeaningfulEvidenceEntry(post.verification_steps,12);return <article key={post.id} data-freshness={evidence.freshness} className={styles.queueItem}><div><span className={styles.state}>{evidence.freshness==='stale'?<FiAlertTriangle/>:<FiCheckCircle/>}{evidence.freshness}</span><h3><Link to={`/posts/${post.slug||post.id}`}>{post.title}</Link></h3><p>{post.environment?.join(' · ')|| (en?'No test environment':'Test ortamı yok')}</p><small>{summary.total?`${summary.total} ${en?'community confirmations':'topluluk doğrulaması'}`:(en?'No community confirmations':'Topluluk doğrulaması yok')}</small><AuthorFailurePanel postId={post.id} count={summary.failed} /></div><div className={styles.actions}><Link to={`/posts/${post.id}/edit`}><FiEdit3/>{en?'Edit evidence':'Kanıtı düzenle'}</Link><button type="button" disabled={reverify.isPending||!canReverify} onClick={()=>reverify.mutate({postId:post.id,reason:'Author re-verified current environment and checks'})}><FiRefreshCw/>{en?'Re-verify now':'Şimdi yeniden doğrula'}</button></div></article>})}</div></section>
